...
This method is called **Stochastic Gradient Descent (SGD)**. "Stochastic", because we are training with random numbers. "Gradient", because we are using the collection of all partial derivatives in the chain. "Descent", because we are truly descending our internal parameters using the computed gradient. 

Our formulas for optimization look like this:
